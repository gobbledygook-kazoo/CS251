...
class graph {
 ...
 public:
  ...
  bool addEdge(VertexT from, VertexT to, WeightT weight) {
    if (!adjList.count(from) || !adjList.count(to)) return false;
  ...
  bool getWeight(VertexT from, VertexT to, WeightT& weight) const {
    if (!adjList.count(from)) return false;
    
    auto it = adjList.at(from).find(to);
    if (it == adjList.at(from).end()) return false;
